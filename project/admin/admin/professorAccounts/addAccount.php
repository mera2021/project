...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  if(empty($name) || empty($email)  || empty($password) || empty($address) || empty($phone) || empty($department) ){
   $ErrorFlag = 1;
  ...
  $ErrorMessage['image'] = "Error in Upload File";
    ...
    $ErrorMessage['image'] = "Error in file extension ";       
        ...
        if($ErrorFlag == 0){
            ...
            $sql = "insert into professors (name,email,password,address,phone,image,department) values('$name','$email','$password','$address','$phone','$imgName','$department')";
